...
func (uu *UserUpdate) AddJournalEntryIDs(ids ...uuid.UUID) *UserUpdate {
	uu.mutation.AddJournalEntryIDs(ids...)
	return uu
}

// AddJournalEntries adds the "journal_entries" edges to the JournalEntry entity.
func (uu *UserUpdate) AddJournalEntries(j ...*JournalEntry) *UserUpdate {
	ids := make([]uuid.UUID, len(j))
	for i := range j {
		ids[i] = j[i].ID
	}
	return uu.AddJournalEntryIDs(ids...)
}
...
func (uu *UserUpdate) ClearJournalEntries() *UserUpdate {
	uu.mutation.ClearJournalEntries()
	return uu
}

// RemoveJournalEntryIDs removes the "journal_entries" edge to JournalEntry entities by IDs.
func (uu *UserUpdate) RemoveJournalEntryIDs(ids ...uuid.UUID) *UserUpdate {
	uu.mutation.RemoveJournalEntryIDs(ids...)
	return uu
}

// RemoveJournalEntries removes "journal_entries" edges to JournalEntry entities.
func (uu *UserUpdate) RemoveJournalEntries(j ...*JournalEntry) *UserUpdate {
	ids := make([]uuid.UUID, len(j))
	for i := range j {
		ids[i] = j[i].ID
	}
	return uu.RemoveJournalEntryIDs(ids...)
}
...
func (uu *UserUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if uu.mutation.JournalEntriesCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   user.JournalEntriesTable,
			Columns: []string{user.JournalEntriesColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
	...
	if nodes := uu.mutation.RemovedJournalEntriesIDs(); len(nodes) > 0 && !uu.mutation.JournalEntriesCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   user.JournalEntriesTable,
	...
	if nodes := uu.mutation.JournalEntriesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   user.JournalEntriesTable,
...
// AddJournalEntryIDs adds the "journal_entries" edge to the JournalEntry entity by IDs.
func (uuo *UserUpdateOne) AddJournalEntryIDs(ids ...uuid.UUID) *UserUpdateOne {
	uuo.mutation.AddJournalEntryIDs(ids...)
	return uuo
}

// AddJournalEntries adds the "journal_entries" edges to the JournalEntry entity.
func (uuo *UserUpdateOne) AddJournalEntries(j ...*JournalEntry) *UserUpdateOne {
	ids := make([]uuid.UUID, len(j))
	for i := range j {
		ids[i] = j[i].ID
	}
	return uuo.AddJournalEntryIDs(ids...)
}
...
// ClearJournalEntries clears all "journal_entries" edges to the JournalEntry entity.
func (uuo *UserUpdateOne) ClearJournalEntries() *UserUpdateOne {
	uuo.mutation.ClearJournalEntries()
	return uuo
}

// RemoveJournalEntryIDs removes the "journal_entries" edge to JournalEntry entities by IDs.
func (uuo *UserUpdateOne) RemoveJournalEntryIDs(ids ...uuid.UUID) *UserUpdateOne {
	uuo.mutation.RemoveJournalEntryIDs(ids...)
	return uuo
}

// RemoveJournalEntries removes "journal_entries" edges to JournalEntry entities.
func (uuo *UserUpdateOne) RemoveJournalEntries(j ...*JournalEntry) *UserUpdateOne {
	ids := make([]uuid.UUID, len(j))
	for i := range j {
		ids[i] = j[i].ID
	}
	return uuo.RemoveJournalEntryIDs(ids...)
}
...
func (uuo *UserUpdateOne) sqlSave(ctx context.Context) (_node *User, err error) {
	...
	if uuo.mutation.JournalEntriesCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   user.JournalEntriesTable,
	...
	if nodes := uuo.mutation.RemovedJournalEntriesIDs(); len(nodes) > 0 && !uuo.mutation.JournalEntriesCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   user.JournalEntriesTable,
	...
	if nodes := uuo.mutation.JournalEntriesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   user.JournalEntriesTable,
