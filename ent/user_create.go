...
func (uc *UserCreate) AddJournalEntryIDs(ids ...uuid.UUID) *UserCreate {
	uc.mutation.AddJournalEntryIDs(ids...)
	return uc
}

// AddJournalEntries adds the "journal_entries" edges to the JournalEntry entity.
func (uc *UserCreate) AddJournalEntries(j ...*JournalEntry) *UserCreate {
	ids := make([]uuid.UUID, len(j))
	for i := range j {
		ids[i] = j[i].ID
	}
	return uc.AddJournalEntryIDs(ids...)
}
...
func (uc *UserCreate) createSpec() (*User, *sqlgraph.CreateSpec) {
	...
	if nodes := uc.mutation.JournalEntriesIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
