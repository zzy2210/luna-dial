...
func (uq *UserQuery) QueryJournalEntries() *JournalEntryQuery {
	query := (&JournalEntryClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withJournalEntries: uq.withJournalEntries.Clone(),
...
func (uq *UserQuery) WithJournalEntries(opts ...func(*JournalEntryQuery)) *UserQuery {
	query := (&JournalEntryClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withJournalEntries = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withJournalEntries; query != nil {
		if err := uq.loadJournalEntries(ctx, query, nodes,
			func(n *User) { n.Edges.JournalEntries = []*JournalEntry{} },
			func(n *User, e *JournalEntry) { n.Edges.JournalEntries = append(n.Edges.JournalEntries, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadJournalEntries(ctx context.Context, query *JournalEntryQuery, nodes []*User, init func(*User), assign func(*User, *JournalEntry)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(journalentry.FieldUserID)
	}
	query.Where(predicate.JournalEntry(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.JournalEntriesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
