...
func (tq *TaskQuery) QueryJournalEntries() *JournalEntryQuery {
	query := (&JournalEntryClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TaskQuery) Clone() *TaskQuery {
	...
	return &TaskQuery{
		...
		withJournalEntries: tq.withJournalEntries.Clone(),
...
func (tq *TaskQuery) WithJournalEntries(opts ...func(*JournalEntryQuery)) *TaskQuery {
	query := (&JournalEntryClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withJournalEntries = query
	return tq
}
...
func (tq *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if query := tq.withJournalEntries; query != nil {
		if err := tq.loadJournalEntries(ctx, query, nodes,
			func(n *Task) { n.Edges.JournalEntries = []*JournalEntry{} },
			func(n *Task, e *JournalEntry) { n.Edges.JournalEntries = append(n.Edges.JournalEntries, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TaskQuery) loadJournalEntries(ctx context.Context, query *JournalEntryQuery, nodes []*Task, init func(*Task), assign func(*Task, *JournalEntry)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(task.JournalEntriesTable)
		s.Join(joinT).On(s.C(journalentry.FieldID), joinT.C(task.JournalEntriesPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(task.JournalEntriesPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(task.JournalEntriesPrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*JournalEntry](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "journal_entries" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
